...
def filter_imgs(gImg):
    ...
    Emboss_img = cv2.filter2D(gImg, cv2.CV_16S, Emboss)
    Motion_img = cv2.filter2D(gImg, cv2.CV_16S, Motion)
    ...
    different_H = np.array([[0, 0, 0],
                            [-1, 1, 0],
                            [0, 0, 0]])
    different_temp = cv2.filter2D(gImg, cv2.CV_16S, different_V)
    ...
    Sobel_H = np.array([[1, 0, -1],
                        [2, 0, -2],
                        [1, 0, -1]])
    Sobel_temp = cv2.filter2D(gImg, cv2.CV_16S, Sobel_V)
    ...
    Prewitt_H = np.array([[-1, 0, 1],
                          [-1, 0, 1],
                          [-1, 0, 1]])
    Prewitt_temp = cv2.filter2D(gImg, cv2.CV_16S, Prewitt_V)
    ...
    lap_filter_P = cv2.filter2D(gImg, cv2.CV_16S, kernel_P)
    ...
    lap_filter_N = cv2.filter2D(gImg, cv2.CV_16S, kernel_N)
    ...
    return (
    Emboss_img, Motion_img, different_img, Sobel_img, Prewitt_img, edge4_img_P, edge8_img_P, edge4_img_N, edge8_img_N)
...
if __name__ == '__main__':
    img = cv2.imread("H:\\left1.png")
    img_raw = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    LoG_img = log_filter(gray_img)
    FilterImgs = [img_raw, LoG_img]
    FilterImgs.extend(filter_imgs(gray_img))
    show(FilterImgs)
